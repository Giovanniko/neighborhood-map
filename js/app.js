//Client ID
var key = "G45XPMYEL50EYHLGTAJFLPHC3JHQVIFE0FF4GVRLA0500JRQ";
//Client Secret
var secret = "JHLNNLWDLGATL4FCWEA3422WWGGSQIC4MXEOVKKGVGFBRZ4B";
var search = 'https://api.foursquare.com/v2/venues/search?client_id=';
var baseUrl = search + key + '&client_secret=' + secret + '&v=20130815';
var oviedoLatLng = {lat: 43.36029, lng: -5.84476};//start position
//global variables
var panorama;
...
$(function() {
	...
	var viewModel = {
		...
		initMap: function() {
			...
			//new instance of searchBox object
			...
			streetViewService.getPanorama({location: oviedoLatLng, radius: 50}, viewModel.googleStreetview);
		...
		googleStreetview: function(data, status) {
			if(status == 'OK') {
				...
				panoMarker.addListener('click', function() {
					...
					(function(e) {
							var buttonHTML = '<button style="margin: 5px 0px;" id="FsqButton2" data-bind="click: incrementClickCounter">Nearby Places</button>';
		...
		detailsPls: function(locale) {
			 ...
			 var apiCall = baseUrl + urlOptions;
			$(function() {
				$.ajax({
					url: apiCall,
					dataType: 'jsonp',
